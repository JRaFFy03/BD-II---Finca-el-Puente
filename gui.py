...
class SistemaGanaderoApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Finca El Puente - Registro Ganadero")
        # Simular una pantalla de teléfono móvil en el escritorio (412x730)
        self.root.geometry("412x730")
        self.root.resizable(False, False)
        self.root.configure(bg="#f4f6f7")

        # Base de datos local simulada (Idéntica a la del index.html)
        self.base_datos = {
            "BHM-0001": {
                "nombre": "Toro Fuerte",
                "raza": "Brahman",
                "peso": "550 Kg",
                "ultimo_peso": "18/05/2025",
                "ultima_vacuna": "15/05/2025",
                "estado": "Saludable"
            },
            "BHM-0002": {
                "nombre": "Vaca Bella",
                "raza": "Gyr",
                "peso": "420 Kg",
                "ultimo_peso": "02/06/2026",
                "ultima_vacuna": "20/05/2026",
                "estado": "Saludable"
            },
            "BHM-0003": {
                "nombre": "Clavel",
                "raza": "Nelore",
                "peso": "490 Kg",
                "ultimo_peso": "14/06/2026",
                "ultima_vacuna": "10/04/2026",
    ...
    def crear_estilos(self):
        ...
        # Configuración de colores (Verde Ganadero)
        self.style.configure("TLabel", background="#ffffff", foreground="#2c3e50", font=("Segoe UI", 11))
        self.style.configure("Card.TFrame", background="#ffffff", borderwidth=1, relief="solid")

    def crear_interfaz(self):
        # 1. Barra superior / Navbar
        navbar = tk.Frame(self.root, bg="#2e7d32", height=60)
        navbar.pack(fill="x", side="top")
        navbar.pack_propagate(False)

        lbl_titulo = tk.Label(navbar, text="Finca El Puente", fg="white", bg="#2e7d32", font=("Segoe UI", 14, "bold"))
        lbl_titulo.pack(side="left", padx=15, pady=15)

        # Contenedor con scroll o margen interno
        self.contenedor_principal = tk.Frame(self.root, bg="#f4f6f7", padx=20, pady=20)
        self.contenedor_principal.pack(fill="both", expand=True)

        # 2. Caja de simulación de Escaneo QR / Entrada de ID
        caja_busqueda = tk.Frame(self.contenedor_principal, bg="#2e7d32", bd=0, highlightbackground="#4caf50", highlightthickness=1)
        caja_busqueda.pack(fill="x", pady=(0, 15))
        
        lbl_instruccion = tk.Label(caja_busqueda, text="Simular Escaneo QR (Ingresa ID):", fg="#ffffff", bg="#2e7d32", font=("Segoe UI", 10, "bold"))
        lbl_instruccion.pack(pady=(10, 2))

        # Campo de texto para el ID
        self.txt_id = entry = tk.Entry(caja_busqueda, justify="center", font=("Segoe UI", 12, "bold"), bd=0, bg="#ffffff", fg="#2c3e50")
        self.txt_id.insert(0, "BHM-0001")
        self.txt_id.pack(fill="x", padx=30, pady=5)

        # Botón Escanear / Consultar
        btn_escanear = tk.Button(
            caja_busqueda, 
            text="🔍 ESCANEAR / CONSULTAR", 
            bg="#4caf50", 
            fg="white", 
            ...
            bd=0, 
            cursor="hand2",
            command=self.consultar_animal
        )
        btn_escanear.pack(fill="x", padx=30, pady=(5, 15))

        # 3. Contenedor Dinámico para la Ficha Técnica (Card)
        self.card_ficha = tk.Frame(self.contenedor_principal, bg="#ffffff", bd=1, highlightbackground="#e0e0e0", highlightthickness=1)
        self.card_ficha.pack(fill="both", expand=True)
        
        # Inicializar con la vista por defecto de carga
        self.mostrar_vista_vacia()

    def mostrar_vista_vacia(self):
        for widget in self.card_ficha.winfo_children():
            widget.destroy()
    ...
    def consultar_animal(self):
        codigo_ingresado = self.txt_id.get().strip().upper()
        
        if not codigo_ingresado:
            messagebox.showwarning("Campo vacío", "Por favor, introduce o simula un ID de animal.")
            return

        # Limpiar contenedor de la ficha
        for widget in self.card_ficha.winfo_children():
            widget.destroy()

        if codigo_ingresado in self.base_datos:
            animal = self.base_datos[codigo_ingresado]
            
            # Título de la Ficha
            lbl_ficha_titulo = tk.Label(
                self.card_ficha, 
                text="FICHA TÉCNICA DEL ANIMAL", 
                bg="#ffffff", 
                fg="#7f8c8d", 
                font=("Segoe UI", 10, "bold")
            )
            lbl_ficha_titulo.pack(anchor="w", padx=15, pady=(15, 5))
            
            # Divisor horizontal simple
            divisor = tk.Frame(self.card_ficha, bg="#f0f0f0", height=2)
            divisor.pack(fill="x", padx=15, pady=(0, 15))

            # Crear las filas ordenadas de atributos
            campos = [
                ("ID Registro:", codigo_ingresado),
                ("Nombre:", animal["nombre"]),
                ("Raza:", animal["raza"]),
                ("Peso Actual:", animal["peso"]),
                ("Último Peso:", animal["ultimo_peso"]),
                ("Última Vacuna:", animal["ultima_vacuna"])
            ]

            for etiqueta, valor in campos:
                fila = tk.Frame(self.card_ficha, bg="#ffffff")
                fila.pack(fill="x", padx=15, pady=6)
                
                lbl_etiq = tk.Label(fila, text=etiqueta, bg="#ffffff", fg="#7f8c8d", font=("Segoe UI", 10, "bold"))
                lbl_etiq.pack(side="left")
                
                lbl_val = tk.Label(fila, text=valor, bg="#ffffff", fg="#2c3e50", font=("Segoe UI", 11, "bold"))
                lbl_val.pack(side="right")

            # Fila especial para el "Estado" con su color distintivo
            fila_estado = tk.Frame(self.card_ficha, bg="#ffffff")
            fila_estado.pack(fill="x", padx=15, pady=12)

            lbl_est_etiq = tk.Label(fila_estado, text="Estado:", bg="#ffffff", fg="#7f8c8d", font=("Segoe UI", 10, "bold"))
            lbl_est_etiq.pack(side="left")

            # Definir color del Badge de estado
            color_badge = "#2ecc71" # Verde saludable
            if animal["estado"].lower() == "en observación":
                color_badge = "#f39c12" # Naranja
            elif animal["estado"].lower() == "enfermo":
                color_badge = "#e74c3c" # Rojo

            lbl_est_val = tk.Label(
                fila_estado, 
                text=f"  {animal['estado']}  ", 
                bg=color_badge, 
                fg="white", 
                font=("Segoe UI", 10, "bold"),
                relief="flat"
            )
            lbl_est_val.pack(side="right")

        else:
            # Vista si el ID escaneado no existe
            lbl_error_icon = tk.Label(self.card_ficha, text="⚠️", bg="#ffffff", fg="#e74c3c", font=("Segoe UI", 28))
            lbl_error_icon.pack(pady=(40, 5))
            
            lbl_error_tit = tk.Label(self.card_ficha, text="No Registrado", bg="#ffffff", fg="#2c3e50", font=("Segoe UI", 12, "bold"))
            lbl_error_tit.pack(pady=5)

            lbl_error_desc = tk.Label(
                self.card_ficha, 
                text=f"El código '{codigo_ingresado}' no\ncoincide con nuestros archivos.", 
                bg="#ffffff", 
                fg="#7f8c8d", 
                font=("Segoe UI", 10),
                justify="center"
            )
            lbl_error_desc.pack(pady=5)

if __name__ == "__main__":
    root = tk.Tk()
    app = SistemaGanaderoApp(root)
    root.mainloop()